...
namespace JsonApiDotNetCore.MongoDb.Example.Tests.IntegrationTests.Pagination
{
    public sealed class PaginationWithoutTotalCountTests : IClassFixture<IntegrationTestContext<Startup>>
    {
        ...
        public async Task When_page_size_is_specified_in_query_string_with_no_data_it_should_render_pagination_links()
        {
            ...
            await _testContext.RunOnDatabaseAsync(async db => await db.GetCollection<Article>(nameof(Article)).DeleteManyAsync(Builders<Article>.Filter.Empty));
        ...
        //     });
        //
        //     var route = $"/authors/{author.StringId}/articles?page[number]=3&foo=bar";
        //
        //     // Act
        //     var (httpResponse, responseDocument) = await _testContext.ExecuteGetAsync<Document>(route);
        //
        ...
        //
        //     responseDocument.Links.Should().NotBeNull();
        //     responseDocument.Links.Self.Should().Be($"http://localhost/authors/{author.StringId}/articles?page[number]=3&foo=bar");
        //     responseDocument.Links.First.Should().Be($"http://localhost/authors/{author.StringId}/articles?foo=bar");
        //     responseDocument.Links.Last.Should().BeNull();
        //     responseDocument.Links.Prev.Should().Be($"http://localhost/authors/{author.StringId}/articles?page[number]=2&foo=bar");
        //     responseDocument.Links.Next.Should().Be($"http://localhost/authors/{author.StringId}/articles?page[number]=4&foo=bar");
        // }
    }
}
